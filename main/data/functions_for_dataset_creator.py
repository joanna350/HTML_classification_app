...
def is_visible(element):
    """
    Checks if an element is visible in the DOM using the visible attribute
    ...
    is_visible : bool
    """
    if element.has_attr("_visible") and (element.attrs["_visible"] == "true"):
        return True
    else:
        return False
...
def html_str_to_one_string_of_visible_text(html_str):
    ...
    list_of_text = text_from_html(elements)

    text_str = " ".join(list_of_text)
    return text_str


def text_from_html(elements):
    """
...
def filter_dict_creator(domain, dict_of_domains):
    """
